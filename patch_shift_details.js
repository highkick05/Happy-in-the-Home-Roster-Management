...
content = content.replace("const [cancelReason, setCancelReason] = React.useState('');", "const [cancelReason, setCancelReason] = React.useState('');\n  const [clientGaveNotice, setClientGaveNotice] = React.useState(true);");
...
content = content.replace(
  ...
  "const body = isRespite ? { status: 'CANCELLED' } : { reason: cancelReason, clientGaveNotice };"
);

const cancelJSXTarget = `<label className="block text-sm md:text-base font-semibold text-zinc-300 mb-2">Reason for Cancellation</label>`;
const newCancelJSX = `<div className="flex items-center justify-between mb-4 bg-zinc-800/50 p-3 rounded-lg border border-white/[0.05]">
