...
content = content.replace("const [cancelReason, setCancelReason] = useState('');", "const [cancelReason, setCancelReason] = useState('');\n  const [clientGaveNotice, setClientGaveNotice] = useState(true);");
content = content.replace("setCancelReason('');", "setCancelReason('');\n      setClientGaveNotice(true);");
...
const cancelJSXTarget = `<label className="block text-sm font-semibold text-zinc-300 mb-2">Reason for Cancellation</label>`;
const newCancelJSX = `<div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 bg-zinc-800/50 p-3 rounded-lg border border-white/[0.05] gap-3">
                  ...
                  <label className="block text-sm font-semibold text-zinc-300 mb-2">Reason for Cancellation</label>`;

content = content.replace(cancelJSXTarget, newCancelJSX);
