...
// 1. Fix the duplicate `const pt_km = row.provider_travel_km || 0;` at the very beginning of the duplicate.
let lines = content.split('\n');
// Let's just find where clientMatrix `const pt_km` started. It's around line 632.
lines.splice(631, 102, correctCellLogic.split('\n').map(l => '                         ' + l)); // wait, 102 is an estimate.

fs.writeFileSync('src/components/Compliance/ComplianceDashboard.tsx.tmp', lines.join('\n'));
