...
const start = lines.findIndex(l => l.includes('<label className="flex items-center space-x-3 cursor-pointer group">'));
const end = lines.findIndex((l, i) => i > start && l.includes('</label>'));
...
if (start !== -1 && end !== -1) {
  lines.splice(start, end - start + 1, checkboxesStr);
  fs.writeFileSync(file, lines.join('\n'), 'utf8');
  console.log('patched exactly by lines');
} else {
