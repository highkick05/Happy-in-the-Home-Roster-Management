...
const oldCodeStart = '  app.get(\n    "/api/admin/staff-compliance",\n    authenticateToken,\n    requireAdmin,';
const oldCodeEnd = 'res.status(500).json({ error: "Internal Server Error" });\n      }\n    },\n  );';

const startIndex = content.indexOf('  app.get(\n    "/api/admin/staff-compliance"');
const endIndex = content.indexOf('  );', startIndex + oldCodeStart.length) + 4;

if (startIndex !== -1 && endIndex !== -1) {
  const newCode = `  app.get("/api/admin/staff-compliance", authenticateToken, requireAdmin, (req: any, res: any) => {
    try {
