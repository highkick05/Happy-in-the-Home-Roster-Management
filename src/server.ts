...
async function startServer() {
  ...
  try {
    db.exec("ALTER TABLE clients ADD COLUMN care_coordination_fee REAL DEFAULT 20.00");
  ...
  try {
    ...
    console.log('[DEBUG] Completed clients.home_care_level_or_class column check.');
  } catch(e: any) {
    if (e.message && !e.message.includes('duplicate column')) {
      console.warn('Migration warning:', e.message);
    }
  ...
  app.get('/api/notifications', authenticateToken, (req: any, res: any) => {
     try {
      ...
      logger.error('API Error', Object.assign({}, e, { message: e?.message, stack: e?.stack }));
...
`Regards,\nHappy in the Home Team`,
  ...
  app.get('/api/pwa-icon/:size', async (req, res) => {
    try {
      ...
      logger.error('API Error', Object.assign({}, err, { message: err?.message, stack: err?.stack }));
      res.status(500).json({ error: 'Internal Server Error' });
    }
  });
  ...
  app.put('/api/clients/:id/ndis-agreements/:agrId/status', authenticateToken, (req, res) => {
    const { id, agrId } = req.params;
    const { status } = req.body;
    try {
      db.prepare(`UPDATE ndis_service_agreements SET status = ? WHERE id = ? AND client_id = ?`).run(status, agrId, id);
      res.json({ success: true });
    } catch (e: any) {
      logger.error(`API Error updating NDIS agreement status: ${e}`, { error: "Internal Server Error" });
      res.status(500).json({ error: 'Internal Server Error' });
    }
  });

  app.delete('/api/clients/:id/ndis-agreements/:agrId', authenticateToken, (req, res) => {
    const { id, agrId } = req.params;
    try {
      db.prepare(`DELETE FROM ndis_service_agreement_items WHERE agreement_id = ?`).run(agrId);
      db.prepare(`DELETE FROM ndis_service_agreements WHERE id = ? AND client_id = ?`).run(agrId, id);
      res.json({ success: true });
    } catch (e: any) {
      logger.error(`API Error deleting NDIS agreement: ${e}`, { error: "Internal Server Error" });
      res.status(500).json({ error: 'Internal Server Error' });
    }
  });
  ...
  app.get('/api/settings/position-templates', authenticateToken, (req, res) => {
    try {
      const templates = db.prepare('SELECT position_title, description_text FROM position_templates').all();
      res.json(templates);
    } catch (e: any) {
  ...
  app.post('/api/staff/:id/generate-contract', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (logoUrl) {
          try {
            ...
            if (require('fs').existsSync(logoPath)) {
              doc.image(logoPath, doc.page.width - 40 - 90, 40, { width: 90 });
      ...
      doc.y = 40;
      // Header
      doc.font('Helvetica-Bold').fontSize(22).fillColor(primaryColor).text('Letter of Offer', { width: doc.page.width - 80 });
      doc.moveDown(1);
      ...
      doc.x = 40;
      doc.y = 40;
      doc.font('Helvetica-Bold').fontSize(22).text('Employment Contract', { width: doc.page.width - 80 });
      doc.moveDown(1.5);
