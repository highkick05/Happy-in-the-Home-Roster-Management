...
export default function ProgressNotesFeed({
  ...
  const renderNoteContent = (contentStr: string) => {
    try {
      ...
      if (data && data.blocks) {
        return (
          <div className="text-[12px] leading-[1.4] text-[#E6EDF3] block [&>p]:mb-1 [&>p]:last:mb-0 [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:pl-5 [&_li]:mb-1">
    ...
    return <div className="text-[12px] text-[#E6EDF3] leading-[1.4] whitespace-pre-wrap block">{contentStr}</div>;
  };
                 ...
                 <div className="mt-2">
                   ...
                   {note.tags && (
                     <div className="mt-3 block pt-2">
                       <span className={`inline-block border px-2 py-1 rounded text-[12px] shadow-sm ${note.tags?.includes('Incident') ? 'bg-red-500/20 border-red-500/30 text-red-400 font-medium' : 'bg-brand-navy border-border-subtle text-zinc-400'}`}>
                         Tag: {note.tags}
                       </span>
                     </div>
                   )}
