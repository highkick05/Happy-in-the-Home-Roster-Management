...
export default function StaffActivityReport() {
  ...
  return (
    <div className="flex flex-col h-full space-y-2 min-h-[calc(100vh-120px)]">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 shrink-0">
        <div>
          <h2 className="text-base font-sans font-semibold text-[#E6EDF3] flex items-center">
            <Activity className="w-4 h-4 mr-2 text-brand-teal" />
            Staff Activity & Service Breakdown
          </h2>
          <p className="text-[#8B949E] text-xs mt-0">Monitor staff hours, services provided, and travel distances</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <select 
            ...
            className="bg-brand-navy border border-border-subtle text-[#E6EDF3] text-xs rounded-md px-2 py-1 focus:outline-none focus:border-brand-teal transition-colors max-w-[200px]"
          >
          ...
          <div className="flex items-center space-x-2 bg-brand-navy border border-border-subtle rounded-md px-2 py-1 focus-within:border-brand-teal transition-colors">
            ...
            <CustomDatePicker
              ...
              className="bg-transparent border-none outline-none text-[#E6EDF3] text-xs"
            />
          ...
          <div className="relative">
            <button 
              ...
              className="flex items-center gap-1 px-2 py-1 bg-brand-navy hover:bg-[#1f262e] text-[#E6EDF3] rounded-md text-xs transition-colors border border-border-subtle"
                    ...
                    {columnsList.map((col) => {
                      ...
                      return (
                        <button
                          ...
                          className="w-full flex items-center gap-2.5 px-3 py-1.5 rounded text-xs text-[#E6EDF3] hover:bg-brand-bg transition-colors text-left"
                        >
                          <input
                            type="checkbox"
                            checked={isChecked}
      ...
      <div className="bg-brand-navy border border-border-subtle rounded-xl overflow-hidden flex flex-col flex-1 shadow-sm">
          ...
          <table className="w-full text-left text-xs text-[#8B949E] relative">
            <thead className="text-[10px] uppercase bg-brand-bg text-[#8B949E] sticky top-0 z-10 font-medium border-b border-border-subtle">
              <tr>
                ...
                {visibleColumns.includes('client') && <th className="px-3 py-2 min-w-[150px]">Client</th>}
                {visibleColumns.includes('serviceProvided') && <th className="px-3 py-2 min-w-[200px]">Service Provided</th>}
                {visibleColumns.includes('dayCategory') && <th className="px-3 py-2 text-center">Day Category</th>}
                {visibleColumns.includes('workHrs') && <th className="px-3 py-2 text-right">Work (Hrs)</th>}
            ...
            <tbody className="divide-y divide-border-subtle">
              {loading && !reportData ? (
                <tr>
                  <td colSpan={visibleColumns.length} className="px-3 py-10 text-center text-[#8B949E]">
                ...
                {visibleColumns.includes('shiftId') && (
                    ...
                    {visibleColumns.includes('time') && (
                      <td className="px-3 py-2 whitespace-nowrap text-[#8B949E] text-xs">{row.timeString}</td>
                    )}
                    {visibleColumns.includes('staffMember') && (
                      <td className="px-3 py-2 text-[#E6EDF3]">{row.staffName}</td>
                    )}
                    {visibleColumns.includes('client') && (
                      <td className="px-3 py-2 text-[#E6EDF3]">{row.clientName}</td>
                    )}
                    ...
                    {visibleColumns.includes('workHrs') && (
                      <td className="px-3 py-2 text-right tabular-nums text-[#E6EDF3] font-semibold">{row.hoursWorked}</td>
                    )}
                    {visibleColumns.includes('shiftDuration') && (
                      <td className="px-3 py-2 text-right tabular-nums text-brand-teal font-medium">{row.shiftDuration !== undefined ? row.shiftDuration : '-'}</td>
                    )}
                    {visibleColumns.includes('travelKm') && (
                      <td className="px-3 py-2 text-right tabular-nums">{row.travelKm}</td>
                    )}
                    {visibleColumns.includes('travelHrs') && (
                      <td className="px-3 py-2 text-right tabular-nums">{row.travelHours !== undefined ? row.travelHours.toFixed(2) : '-'}</td>
                    )}
                    {visibleColumns.includes('travelReimbursement') && (
                      <td className="px-3 py-2 text-right tabular-nums">{row.travelReimbursement !== undefined ? `$${row.travelReimbursement.toFixed(2)}` : '-'}</td>
                    )}
                  </tr>
                ))
              )}
        ...
        {reportData && (
          <div className="bg-brand-bg border-t border-border-subtle p-5 mt-auto shrink-0 animate-fade-in">
            <h3 className="text-xs font-semibold text-[#E6EDF3] uppercase tracking-wider mb-4 flex items-center">
            ...
            <div className="grid grid-cols-2 md:grid-cols-7 gap-4">
              <div className="bg-brand-navy rounded-lg p-3 border border-border-subtle">
                ...
                <div className="text-lg font-bold text-[#E6EDF3] tabular-nums">{reportData.totals.weekdayHours} <span className="text-xs font-normal text-[#8B949E]">hrs</span></div>
              </div>
              <div className="bg-brand-navy rounded-lg p-3 border border-border-subtle">
                <div className="text-[#8B949E] text-xs font-semibold mb-1 uppercase tracking-wide">Total Saturday Hrs</div>
                <div className="text-lg font-bold text-[#E6EDF3] tabular-nums">{reportData.totals.saturdayHours} <span className="text-xs font-normal text-[#8B949E]">hrs</span></div>
              </div>
              <div className="bg-brand-navy rounded-lg p-3 border border-border-subtle">
                <div className="text-[#8B949E] text-xs font-semibold mb-1 uppercase tracking-wide">Total Sunday Hrs</div>
                <div className="text-lg font-bold text-[#E6EDF3] tabular-nums">{reportData.totals.sundayHours} <span className="text-xs font-normal text-[#8B949E]">hrs</span></div>
              </div>
              <div className="bg-brand-navy rounded-lg p-3 border border-border-subtle">
                <div className="text-[#8B949E] text-xs font-semibold mb-1 uppercase tracking-wide">Total Pub Hol Hrs</div>
                <div className="text-lg font-bold text-[#E6EDF3] tabular-nums">{reportData.totals.publicHolidayHours} <span className="text-xs font-normal text-[#8B949E]">hrs</span></div>
              </div>
              <div className="bg-gradient-to-br from-brand-teal/10 to-brand-green/10 rounded-lg p-3 border border-brand-teal/20">
                ...
                <div className="text-lg font-bold text-brand-teal tabular-nums">{reportData.totals.travelKm} <span className="text-xs font-normal text-brand-teal/70">km</span></div>
              </div>
              <div className="bg-gradient-to-br from-brand-teal/10 to-brand-green/10 rounded-lg p-3 border border-brand-teal/20">
                ...
                <div className="text-lg font-bold text-brand-teal tabular-nums">{reportData.totals.travelHrs !== undefined ? reportData.totals.travelHrs : 0} <span className="text-xs font-normal text-brand-teal/70">hrs</span></div>
              </div>
