        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
          <div className="bg-[#121214] border border-white/[0.08] rounded-xl shadow-2xl max-w-3xl w-full flex flex-col">
            ...
            <form onSubmit={handleUploadSubmit} className="p-5 space-y-3 bg-[#09090b]">
              ...
              <div>
                <label className="block text-[11px] font-medium text-zinc-400 mb-1.5">Staff (Optional, multi-select)</label>
                <select
                  multiple
                  value={uploadStaffIds}
                  onChange={e => {
                    const selected = Array.from(e.target.selectedOptions, option => option.value);
                    setUploadStaffIds(selected);
                  }}
                  className="w-full bg-black/40 border border-white/[0.08] rounded-md px-3 py-1.5 text-xs text-white outline-none focus:border-brand-blue transition-colors custom-scrollbar"
                ...
                <p className="text-[10px] text-zinc-500 mt-1">Hold Ctrl (Windows) or Cmd (Mac) to select multiple.</p>
              </div>
              <div>
