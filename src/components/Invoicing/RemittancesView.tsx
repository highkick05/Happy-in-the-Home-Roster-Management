...
function GenerateRemittanceForm({ token, onGenerated, onClose, editData }: { token: string | null, onGenerated: () => void, onClose: () => void, editData?: any }) {
  ...
  const [formData, setFormData] = useState(() => {
    if (editData) {
      ...
      return {
        ...
        endDate: endDate,
        remittanceDate: editData.remittance_date || new Date().toISOString().split('T')[0],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const url = editData ? `/api/remittances/${editData.id}` : '/api/remittances';
  ...
  return (
    <form onSubmit={handleSubmit} className="p-5 space-y-3">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Remittance Date</label>
          <CustomDatePicker
            position="bottom"
            required
      ...
      <div className="pt-2 border-t border-white/[0.08] flex justify-between items-center space-x-3 mt-4">
        ...
        >
          {submitting ? (editData ? 'Saving...' : 'Generating...') : (editData ? 'Save Changes' : 'Generate Remittance')}
        </button>
      </div>
    </form>
  );
...
export default function RemittancesView() {
  ...
  const handleUploadSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/remittances/historical', {
        method: 'POST',
        headers: {
      ...
      if (!res.ok) throw new Error('Failed to upload historical remittance');
  ...
  const sortedRemittances = [...filteredRemittances].sort((a, b) => {
    ...
    if (sortField === 'date') {
      ...
      valA = (a.remittance_number || '').toLowerCase();
      valB = (b.remittance_number || '').toLowerCase();
  ...
  const totalPages = Math.ceil(sortedRemittances.length / pageSize);
  const paginatedRemittances = sortedRemittances.slice((page - 1) * pageSize, page * pageSize);
  ...
  const toggleAll = () => {
    if (selectedIds.length === filteredRemittances.length && paginatedRemittances.length > 0) setSelectedIds([]);
    else setSelectedIds(filteredRemittances.map(q => q.id));
  };

  const handleDeleteBulk = async () => {
    if (selectedIds.length === 0) return;
    if (!confirm(`Are you sure you want to delete ${selectedIds.length} remittances?`)) return;
    try {
      const res = await fetch('/api/remittances/bulk-delete', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ remittanceIds: selectedIds })
      });
      if (res.ok) fetchRemittances();
  ...
  const handleDeleteSingle = async (id: number) => {
    ...
    try {
      const res = await fetch(`/api/remittances/${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } });
      if (res.ok) fetchRemittances();
    } catch (e) { console.error(e); }
  };
  ...
  const downloadPDF = async (id: number, numberStr: string) => {
    try {
      const res = await fetch(`/api/remittances/${id}/download`, { headers: { Authorization: `Bearer ${token}` } });
    ...
    <div className="flex-1 flex flex-col space-y-3">
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
          <div className="bg-[#121214] border border-white/[0.08] rounded-xl shadow-2xl max-w-3xl w-full flex flex-col">
            <div className="p-5 border-b border-white/[0.08] flex justify-between items-center bg-[#18181b] rounded-t-xl shrink-0">
              <h2 className="text-base font-sans font-semibold text-[#E6EDF3] tracking-tight mb-0">Upload Historical Remittance</h2>
            ...
            <form onSubmit={handleUploadSubmit} className="p-5 space-y-3 bg-[#09090b]">
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="e.g. Historical Remittance"
              ...
              <div>
                <label className="block text-[11px] font-medium text-zinc-400 mb-1.5">Remittance PDF</label>
      ...
      {showGenerateModal && (
        <div className="fixed inset-0 z-[60] flex justify-center p-4 bg-black/60 backdrop-blur-sm overflow-y-auto custom-scrollbar">
          <div className="bg-brand-navy border border-border-subtle rounded-xl shadow-2xl w-full max-w-[1400px] flex flex-col h-fit my-auto" onClick={(e) => e.stopPropagation()}>
            <div className="p-4 border-b border-border-subtle flex justify-between items-center bg-brand-bg shrink-0">
              <div>
                <h3 className="text-sm font-medium text-[#E6EDF3] mb-2">{editingRemittance ? 'Edit Service Remittance' : 'Generate Service Remittance'}</h3>
                <p className="text-xs text-[#8B949E] mt-0">Configure service and cost details for a new remittance.</p>
              </div>
            ...
            <div className="overflow-visible flex-1">
              <GenerateRemittanceForm token={token} onGenerated={() => fetchRemittances()} onClose={() => { setShowGenerateModal(false); setEditingRemittance(null); }} editData={editingRemittance} />
            </div>
          </div>
        </div>
      )}
        ...
        <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
          ...
          <button
            onClick={() => { setEditingRemittance(null); setShowGenerateModal(true); }}
...
<div className="flex-1 bg-brand-navy border border-border-subtle rounded-xl overflow-x-auto flex flex-col shadow-sm">
        <div className="flex-1 overflow-auto">
          {loading ? (
            <div className="p-8 text-center text-[#8B949E]">Loading remittances...</div>
          ) : (
            <table className="w-full text-left border-collapse text-xs">
                ...
                <tr className="bg-brand-bg border-b border-border-subtle text-xs uppercase tracking-wider text-[#8B949E] sticky top-0 z-10 transition-colors">
                  <th className="px-3 py-1.5 font-semibold text-[10px] w-12">
                    <input type="checkbox" className="rounded border-border-subtle bg-brand-navy text-brand-teal focus:ring-brand-teal focus:ring-offset-brand-navy" checked={selectedIds.length === paginatedRemittances.length && paginatedRemittances.length > 0} onChange={toggleAll} />
                  </th>
                  {renderSortableHeader('date', 'Date')}
                  {renderSortableHeader('id', 'Remittance ID')}
              ...
              <tbody className="divide-y divide-border-subtle">
                {paginatedRemittances.length === 0 ? (
                  <tr><td colSpan={8} className="p-8 text-center text-[#8B949E]">No remittances found.</td></tr>
                ) : paginatedRemittances.map(q => (
                  <tr key={q.id} className="hover:bg-brand-bg/50 transition-colors group">
                    ...
                    <td className="px-3 py-1.5 whitespace-nowrap font-medium text-[#E6EDF3]">
                      <div className="flex items-center space-x-2 group/copy">
                        <span>{q.remittance_number}</span>
                        <button onClick={(e) => { e.stopPropagation(); handleCopy(q.remittance_number); }} className="opacity-0 group-hover/copy:opacity-100 text-[#8B949E] hover:text-[#E6EDF3] transition-all">
                          {copiedText === q.remittance_number ? <CheckCircle className="w-3.5 h-3.5 text-brand-green" /> : <Copy className="w-3.5 h-3.5" />}
                        </button>
                      </div>
                    </td>
                    ...
                    <td className="px-3 py-1.5 text-right flex items-center justify-end space-x-1">
                      <button title="Edit Remittance" onClick={() => { setEditingRemittance(q); setShowGenerateModal(true); }} className="p-1.5 text-zinc-400 hover:text-blue-400 hover:bg-blue-400/10 rounded-md transition-colors"><Edit2 className="w-4 h-4" /></button>
        ...
        {/* Pagination Controls */}
        {!loading && sortedRemittances.length > 0 && (
          <div className="flex items-center justify-between px-3 py-1.5 border-t border-border-subtle bg-brand-bg">
            ...
            <div className="flex items-center space-x-4 text-sm text-[#8B949E]">
              <span>
                {((page - 1) * pageSize) + 1}-{Math.min(page * pageSize, sortedRemittances.length)} of {sortedRemittances.length}
              </span>
              <div className="flex items-center space-x-1">
                <button 
