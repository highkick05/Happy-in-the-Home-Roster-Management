    ...
    <div className="space-y-3">
                               ...
                               <div className="flex items-center gap-1">
                                 ...
                                 {row.odometer_start_photo && (
                                   <div className="relative group flex items-center">
                                     <button 
                                       onClick={() => setPreviewPhoto({url: row.odometer_start_photo, type: 'Start'})} 
                                       onMouseEnter={(e) => {
                                          const rect = e.currentTarget.getBoundingClientRect();
                                          setHoveredPhoto({url: row.odometer_start_photo, type: 'Start', x: rect.left + rect.width/2, y: rect.top});
                                       }}
                             ...
                             <td className="px-4 py-2 border-r border-border-subtle/30 whitespace-nowrap font-mono text-xs">
                               <div className="flex items-center gap-1">
                                 ...
                                 {row.odometer_end_photo && (
                                   <div className="relative group flex items-center">
                                     <button 
                                       onClick={() => setPreviewPhoto({url: row.odometer_end_photo, type: 'End'})} 
                                       onMouseEnter={(e) => {
                                          const rect = e.currentTarget.getBoundingClientRect();
                                          setHoveredPhoto({url: row.odometer_end_photo, type: 'End', x: rect.left + rect.width/2, y: rect.top});
                                       }}
                                       onMouseLeave={() => setHoveredPhoto(null)}
                                       className="text-[#8B949E] hover:text-[#E6EDF3] transition-colors"
                                     >
      ...
      {activeTab === 'mandatory_documents' && (
        <div className="bg-brand-navy border border-border-subtle rounded-xl shadow-sm overflow-hidden p-0">
          <div className="px-4 py-2.5 border-b border-border-subtle flex flex-col lg:flex-row lg:items-center justify-between gap-3 relative bg-brand-bg">
                  ...
                  {filteredCompliance.map(staff => {
                    ...
                    return (
                      <div key={staff.id} className="transition-all hover:bg-brand-bg/10">
                        ...
                        {isExpanded && (
                          <div className="p-4 bg-[#0E0E10] border-t border-border-subtle shadow-inner">
                            <div className="mb-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                              {Object.entries(ONBOARDING_STEP_LABELS).map(([key, label]) => {
                                ...
                                return (
                                  <div key={key} className={`px-4 py-3 rounded-lg border flex flex-col justify-between h-full min-h-[120px] shadow-sm ${boxStyle}`}>
                                    <div>
                                      <div className="flex items-start justify-between gap-2 mb-2">
                                        <span className="text-[11px] font-semibold text-[#E6EDF3] leading-tight">{label}</span>
      ...
      <div className="bg-brand-navy border border-border-subtle rounded-xl shadow-sm overflow-x-auto">
        <div className="px-4 py-2.5 border-b border-border-subtle bg-brand-bg flex flex-col lg:flex-row lg:items-center justify-between gap-3 relative">
             <div className="flex-1">
