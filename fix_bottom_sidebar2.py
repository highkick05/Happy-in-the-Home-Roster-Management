...
pattern = re.compile(r'<div className={`p-4 border-t border-border-subtle space-y-2 shrink-0 z-10 relative bg-brand-navy \${isDesktopSidebarCollapsed && !isMobileMenuOpen \? \'!px-2\' : \'\'}`\}>.*?<LogOut className=\{`w-5 h-5 \$\{isDesktopSidebarCollapsed && !isMobileMenuOpen \? \'\' : \'mr-3\'\}`\} /> \{!isDesktopSidebarCollapsed \|\| isMobileMenuOpen \? \'Sign Out\' : \'\'\}\n          </button>\n        </div>', re.DOTALL)
