...
content = content.replace(
    "const km = (row.funding_type === 'HOME_CARE' || row.funding_type === 'Home Care' || row.funding_type === 'HCP' ? (row.home_care_travel_km || row.provider_travel_km || 0) : (row.provider_travel_km || 0)) + (row.abt_km || 0);",
