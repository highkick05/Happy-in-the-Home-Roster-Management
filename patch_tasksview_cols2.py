...
# Remove COLUMNS
code = re.sub(r'const COLUMNS = \[.*?\];\n', '', code)

# Replace the grid area
old_grid = """            <div className="grid grid-cols-1 md:grid-cols-3 h-full gap-3 items-start min-w-[700px]">
              {COLUMNS.map(col => {
                const colTasks = tasks.filter(t => t.status === col);
                return (
                  <div key={col} className="flex flex-col w-full max-h-full bg-black/20 rounded-none p-2">
                    <div className="flex flex-col mb-2 px-1">
                      ...
                      <button onClick={() => { setEditingTask({ status: col }); setIsModalOpen(true); }} className="flex items-center gap-2 w-full px-3 py-2 text-[14px] font-medium text-[#8B949E] bg-white/[0.02] hover:bg-white/[0.05] border border-transparent hover:border-white/[0.05] rounded-none transition-colors">
                    ...
                    <Droppable droppableId={col}>
                      {(provided, snapshot) => (
                        <div
                          ...
                          className={`flex-1 overflow-y-auto rounded-none p-2 min-h-[150px] transition-colors ${snapshot.isDraggingOver ? 'bg-white/[0.03] border-white/[0.05]' : 'bg-transparent border-transparent'}`}
                        >"""

new_grid = """            <div className="flex h-full gap-3 items-start min-w-max">
